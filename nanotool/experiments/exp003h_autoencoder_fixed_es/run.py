#!/usr/bin/env python3
"""
Experiment 003h: Autoencoder Test with Fixed ES

Test with corrected ES hyperparameters:
- sigma=0.5 (larger perturbations)
- lr=0.5
- antithetic=False (rank shaping conflicts with antithetic)
"""
...
def main():
    print("=" * 60)
    print("Experiment 003h: Autoencoder (Fixed ES)")
    print("=" * 60)

    # Configuration - corrected hyperparameters
    num_digits = 4
    hidden_dim = 64
    batch_size = 256
    population_size = 100
    sigma = 0.5  # Larger perturbations work better
    learning_rate = 0.5
    weight_decay = 0.0
    ...
    print(f"\nDevice: {device}")
    print(f"sigma={sigma}, lr={learning_rate}, pop={population_size}")
    print(f"antithetic=False (important!)")
    ...
    print("SUCCESS!" if final_acc > 0.9 else ("PARTIAL" if final_digit > 0.9 else "FAILURE"))
