...
def safe_wandb(fn, *args, **kwargs):
    try:
        ...
        print0(f"[wandb disabled] {e}")
        # optionally: set a global flag to stop future attempts
...
def main(config_path_or_obj: Path | str | Config | None = None, **kwargs: Any) -> None:
    ...
    for step in range(1, config.num_iterations + 1):
        ...
        if config.val_loss_every > 0 and (step % config.val_loss_every == 0 or last_step):
            ...
            # log to wandb
            if config.wandb_project is not None and master_process:
                safe_wandb(log_metrics, step, {"val_loss": val_loss})
        ...
        if (
            ...
            if config.wandb_project is not None and master_process:
                ...
                safe_wandb(log_generations, step, generations)
        ...
        for micro_step in range(grad_accum_steps):
            ...
            try:
                ...
                # Rewind the dataset and keep going
                train_loader = iter(create_data_loader(
                    dataset_config=config.train_dataset_config,
                    batch_size=B,
                    buffer_size=1000,
                    global_seed=0,
        ...
        # log to wandb
        if config.wandb_project is not None and master_process:
            safe_wandb(log_metrics, step, {"train_loss": lossf, "lr": lr})
        # log to logile
        if master_process and logfile is not None:
            with open(logfile, "a") as f:
