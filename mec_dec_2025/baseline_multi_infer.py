"""
Baseline inference script for multi-class semantic segmentation.

Applies a baseline U-Net model (no pre-training) to a CloudVolume, processing
one Z-slice at a time with overlapping patches. Trims edge predictions to avoid
boundary artifacts.
"""
...
def extract_patches(image_slice, positions, patch_size):
    ...
    Returns:
        Tensor of patches [N, 1, patch_size, patch_size], normalized to [0, 1]
    """
    patches = []
...
@hydra.main(version_base=None, config_path=".", config_name="baseline_multi_infer_config")
def main(cfg: DictConfig):
    print("="*80)
    print("Baseline Multi-Class Semantic Segmentation Inference")
    print("="*80)
    ...
    # Load model
    print(f"\nLoading baseline model from {cfg.checkpoint}")
    ...
    val_miou_str = f"{checkpoint['val_mean_iou']:.4f}" if 'val_mean_iou' in checkpoint else 'N/A'
