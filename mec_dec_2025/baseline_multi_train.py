"""
Baseline training script for multi-class semantic segmentation with hierarchical supervision.
Uses standard U-Net without pre-training for comparison.
"""
...
from baseline_multi_unet_arch import BaselineMultiUNet
...
def save_predictions(dataloader, model, epoch, output_dir="baseline_multi_predictions", num_samples=4):
    """
...
@hydra.main(version_base=None, config_path=".", config_name="baseline_multi_config")
def main(cfg: DictConfig):
    ...
    if wandb_run_id:
        wandb.init(project="BaselineMultiSeg_Dec2025", id=wandb_run_id, resume="allow", config=dict(cfg))
    else:
        wandb.init(project="BaselineMultiSeg_Dec2025", config=dict(cfg))
    ...
    # Optimizer (all parameters trainable in baseline)
    ...
    checkpoint_dir = "checkpoints_baseline_multi"
